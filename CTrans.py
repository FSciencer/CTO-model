...
class Channel_Embeddings(nn.Module):
    ...
    """
    def __init__(self, config, featuresize, patchsize, in_channels):  # in_channels: [64, 256, 512, 1024], patchsize = [16, 8, 4, 2], featuresize: 512
        super().__init__()
        feature_size = _pair(featuresize)  # [256, 256]
        patch_size = _pair(patchsize)  # [16, 16]
        n_patches = (feature_size[0] // patch_size[0]) * (feature_size[1] // patch_size[1])  # 16*16

        self.patch_embeddings = Conv2d(in_channels=in_channels,  # [64, 256, 256] -> [64, 16, 16]
                                       out_channels=in_channels,
                                       kernel_size=patch_size,
                                       stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches, in_channels))  # [1, 256, 64]
        self.dropout = Dropout(config.transformer["embeddings_dropout_rate"])

    def forward(self, x):
        if x is None:
            return None
        x = self.patch_embeddings(x)  # (B, hidden. n_patches^(1/2), n_patches^(1/2)); [batch, 64, 16, 16]
        x = x.flatten(2)  # [batch, 64, 16*16]
        x = x.transpose(-1, -2)  # (B, n_patches, hidden); [batch, 16*16, 64]
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, featureSize=512, patchSize=[32, 16, 8, 4], channel_num=[64, 128, 256, 512]):  # channel_num: [64, 256, 512, 1024], config.patch_sizes = [16, 8, 4, 2]
        super().__init__()

        self.embeddings_1 = Channel_Embeddings(config, featureSize,    patchSize[0], in_channels=channel_num[0])
        self.embeddings_2 = Channel_Embeddings(config, featureSize//2, patchSize[1], in_channels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(config, featureSize//4, patchSize[2], in_channels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(config, featureSize//8, patchSize[3], in_channels=channel_num[3])
